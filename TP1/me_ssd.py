    ...
    if lamb == 0.:
        for r in range(0, n,
            ...
            for c in range(0, p, bcol):
                ...
                mvf[r:r + brow, c:c + bcol, 0] = np.ones((brow, bcol)) * Dbest[0]  # Once the loop is over, save the best row displacement field
                mvf[r:r + brow, c:c + bcol, 1] = np.ones((brow, bcol)) * Dbest[1]  # Once the loop is over, save the best column displacement field
        ...
        for r in range(0, n,
            ...
            for c in range(0, p, bcol):
                ...
                # Initialization:
                B_init = ref_extended[r + extension:r + brow + extension, c + extension:c + bcol + extension]
                costMin = LA.norm(B - B_init) ** 2 + lamb * (LA.norm(np.array([0, 0]) - pV) ** 2)
                ...
                for dcol in range(-search, +search):  # dcol = candidate displacement vector over the columns
                    for drow in range(-search, +search):  # rcol = candidate displacement vector over the rows
                        ...
                        data_att = LA.norm(B - B_test) ** 2
                        cost = data_att + lamb * (LA.norm(
                            np.array([drow, dcol]) - pV) ** 2)**2  # here, encore the REGULARIZED cost function
