...
def iou_batch(bb_test, bb_gt):
    ...
    for i in range(len(bb_test)):
        ...
        for j in range(len(bb_gt)):
            ...
            area1 = (bb_test[i][2] - bb_test[i][0] + 1) * (bb_test[i][3] - bb_test[i][1] + 1)
            area2 = (bb_gt[j][2] - bb_gt[j][0] + 1) * (bb_gt[j][3] - bb_gt[j][1] + 1)
...
class KalmanBoxTracker(object):
    ...
    def __init__(self, bbox):
        ...
        self.kf.F = np.eye(7)+ np.eye(7, k = 5)
        self.kf.H = np.eye(4, M=7)
